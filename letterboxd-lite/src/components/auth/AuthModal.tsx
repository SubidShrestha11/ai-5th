...
export function AuthModal() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isLogin) {
      ...
      if (result.success) {
        addToast('success', `Welcome back, ${form.username}!`);
        closeAuthModal();
        resetForm();
      } else {
      ...
      const result = register(form.username, form.displayName, form.email, form.password);
      if (result.success) {
        addToast('success', `Welcome to Letterboxd Lite, ${form.displayName}!`);
        closeAuthModal();
        resetForm();
      } else {
        setErrors({ username: result.error ?? 'Registration failed' });
      }
    }
